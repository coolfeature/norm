...
delete(Record) when is_tuple(Record) ->
  Name = norm_utils:record_name(Record),
...
sql_insert(Record,RecordDef,[F|Fs],Insert,Value) ->
  Type = norm_utils:get_value(type,get_value(F,RecordDef),undefined),
  ...
  Where = lists:foldl(fun(Cond,Acc) -> 
    Acc ++ case Cond of
      {Field,Op,Val} when is_atom(Field)->
        ...
        Type = norm_utils:get_value('type',get_value(Field,RecordDef),undefined),
        value_to_string(Field) ++ " " ++  
        value_to_string(Op) ++ " " ++  
...
results_to_record(Record,RecordDef,[Field|Fields],PropList) ->
  ...
  BinVal = norm_utils:get_value(FieldBin,PropList,undefined),
  Type = norm_utils:get_value('type',get_value(Field,RecordDef),undefined),
...
format_to_sql({Decimal,Opts},Value) when 
  ...
  Decimals = norm_utils:get_value(scale,Opts,0),
  value_to_string({Value,Decimals});
format_to_sql({'time',_Opts},Value) ->
  "'" ++ norm_utils:format_time(Value,'iso8601') ++ "'";
format_to_sql({'date',_Opts},Value) ->
  "'" ++ norm_utils:format_date(Value,'iso8601') ++ "'";
format_to_sql({'timestamp',_Opts},Value) ->
  "'" ++ norm_utils:format_datetime(Value,'iso8601') ++ "'";
format_to_sql(_Quoted,Value) ->
  "'" ++ value_to_string(Value) ++ "'".


options_to_string({field,{T,F}},FldOpts) ->
  Type = options_to_string(type,norm_utils:get_value(type,FldOpts,{'bigint',[]})),
  Type ++ " " ++ options_to_string({constraints,{T,F}},
    norm_utils:get_value(constraints,FldOpts,undefined));
...
options_to_string({constraints,{_T,_F}},{references,Opts}) ->
  Ref = norm_utils:get_value('table',Opts,undefined),
  OnDelete = norm_utils:get_value('on_delete',Opts,undefined),
  OnUpdate = norm_utils:get_value('on_update',Opts,undefined),
...
options_to_string('varchar',Opts) ->
  Length = norm_utils:get_value('length',Opts,undefined),
...
options_to_string(Decimal,Opts) when
    ...
    orelse Decimal =:= 'float'
    orelse Decimal =:= 'numeric' -> 
  Precision = norm_utils:get_value(precision,Opts,undefined),
  Scale = norm_utils:get_value(scale,Opts,undefined),
...
get_schema() ->
  case norm_utils:get_db_config(pgsql,tablespace) of
    undefined -> norm;
    Schema -> Schema
  end.

get_pool() ->
  Pools = norm_utils:get_db_config(pgsql,pools),
