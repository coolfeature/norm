...
start_pools(Dbs) ->
  lists:foldl(fun({Name,_Config},Acc) -> 
    Pools = norm_utils:get_db_config(Name,pools),
...
start_db_pools(Db,Pools) ->
  ...
  lists:map(fun({Name,Args}) ->
    SizeArgs = norm_utils:get_value(pool_size,Args,undefined),
    WorkerArgs = norm_utils:get_value(worker_args,Args,undefined),
...
run_init(Dbs) ->
  lists:foldl(fun({Name,Config},Acc) -> 
    InitResult = case norm_utils:get_value(init,Config,false) of
      true -> norm:init(Name);
      _ -> {Name,no_init}
    end,  
    Acc ++ [InitResult]
